...
def _expand_norm_values(values, *, to_rgb, field_name):
    ...
    if np.isscalar(values):
        return [float(values)] * expected_channels
    ...
    if isinstance(values, (list, tuple)):
        ...
        if len(parsed) == expected_channels:
            return parsed
        if len(parsed) == 1:
            return parsed * expected_channels

    raise ValueError(
        f"data.{field_name} must be a scalar or a list of length 1/{expected_channels}"
    )
...
def _resolve_norm_stats(normalize, to_rgb, norm_mean=None, norm_std=None):
    ...
    if norm_mean is not None or norm_std is not None:
        ...
        mean = _expand_norm_values(norm_mean, to_rgb=to_rgb, field_name="mean")
        std = _expand_norm_values(norm_std, to_rgb=to_rgb, field_name="std")
        return mean, std
...
def _compute_sampling_class_weights(class_counts, formula, *, alpha=None, max_weight=None):
    ...
    if formula_norm in {"balanced", "inverse_freq", "inverse", "1/n", "1/n_c"}:
    ...
    elif formula_norm in {"inverse_pow", "inverse_power", "pow"}:
        exponent = 1.0 if alpha is None else float(alpha)
        ...
        raise ValueError(
            ...
            "balanced, inverse_sqrt, inverse_pow, uniform"
        )

    weights = weights / weights.mean()
    if max_weight is not None:
        weights = np.clip(weights, a_min=None, a_max=float(max_weight))
...
def get_dataloaders(data_config, use_cuda, *, build_test=True):
    ...
    norm_mean = data_config.get("mean", None)
    norm_std = data_config.get("std", None)
    ...
    legacy_sampler_cfg = data_config.get("sampler", {})
    legacy_sampler_enabled = bool(legacy_sampler_cfg.get("enabled", False))
    legacy_sampler_alpha = float(legacy_sampler_cfg.get("alpha", 0.5))
    legacy_sampler_max_weight = legacy_sampler_cfg.get("max_weight", None)

    sampler_mode = str(data_config.get("sampler_mode", "")).lower().strip()
    if not sampler_mode:
        sampler_mode = (
            "weighted_random"
            if (imbalance_cfg.get("use_weighted_sampler", False) or legacy_sampler_enabled)
            else "natural"
        )
    ...
    if sampler_mode == "weighted_random":
        use_legacy_sampler = (
            legacy_sampler_enabled
            and "sampler_mode" not in data_config
            and "class_weight_formula" not in data_config
            and not imbalance_cfg.get("use_weighted_sampler", False)
        )
        if use_legacy_sampler:
            sampling_weights = _compute_sampling_class_weights(
                class_counts,
                "inverse_pow",
                alpha=legacy_sampler_alpha,
                max_weight=legacy_sampler_max_weight,
            )
            logging.info(
                "  - Using legacy weighted sampler (alpha=%s, max_weight=%s)",
                legacy_sampler_alpha,
                legacy_sampler_max_weight,
            )
        else:
